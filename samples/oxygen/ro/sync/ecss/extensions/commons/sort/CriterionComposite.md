# Class: `CriterionComposite`

**Package:** [`ro.sync.ecss.extensions.commons.sort`](README.md)

**Fully Qualified Name:** `ro.sync.ecss.extensions.commons.sort.CriterionComposite`

## Fields

### `addCheckbox`

**Type:** `boolean`

### `keyCheckbox`

**Type:** `org.eclipse.swt.widgets.Button`

### `keyCombo`

**Type:** `org.eclipse.swt.widgets.Combo`

### `typeCombo`

**Type:** `org.eclipse.jface.viewers.ComboViewer`

### `orderCombo`

**Type:** `org.eclipse.jface.viewers.ComboViewer`

### `keysController`

**Type:** [`ro.sync.ecss.extensions.commons.sort.KeysController`](./KeysController.md)

### `allCriteria`

**Type:** `java.util.List<ro.sync.ecss.extensions.commons.sort.CriterionInformation>`

## Constructors

### `<init>(Composite arg0, AuthorResourceBundle arg1, List<CriterionInformation> arg2, CriterionInformation arg3, boolean arg4, KeysController arg5, List<CriterionInformation> arg6)`

**Parameters:**
- `arg0` (`org.eclipse.swt.widgets.Composite`)
- `arg1` ([`ro.sync.ecss.extensions.api.AuthorResourceBundle`](../../api/AuthorResourceBundle.md))
- `arg2` (`java.util.List<ro.sync.ecss.extensions.commons.sort.CriterionInformation>`)
- `arg3` ([`ro.sync.ecss.extensions.commons.sort.CriterionInformation`](./CriterionInformation.md))
- `arg4` (`boolean`)
- `arg5` ([`ro.sync.ecss.extensions.commons.sort.KeysController`](./KeysController.md))
- `arg6` (`java.util.List<ro.sync.ecss.extensions.commons.sort.CriterionInformation>`)

## Methods

### `getKeyCombo()`

**Returns:** `org.eclipse.swt.widgets.Combo`

### `getInformation()`

**Returns:** [`ro.sync.ecss.extensions.commons.sort.CriterionInformation`](./CriterionInformation.md)

### `enableSortcriterion()`

**Returns:** `void`

### `changeControlsState()`

**Returns:** `void`

