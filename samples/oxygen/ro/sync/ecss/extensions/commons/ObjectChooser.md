# Class: `ObjectChooser`

**Package:** [`ro.sync.ecss.extensions.commons`](README.md)

**Fully Qualified Name:** `ro.sync.ecss.extensions.commons.ObjectChooser`

## Description

## Fields

### `LOGGER`

**Type:** `org.slf4j.Logger`

### `ALLOWED_IMAGE_EXTENSIONS`

**Type:** `java.lang.String[]`

## Constructors

### `<init>()`

## Methods

### `makeUrlRelative(AuthorAccess authorAccess, String url)`

**Parameters:**
- `authorAccess` ([`ro.sync.ecss.extensions.api.AuthorAccess`](../api/AuthorAccess.md)): The author access of the XML document.
- `url` (`java.lang.String`): The url.

**Returns:** `java.lang.String`

### `getBaseURL(AuthorAccess authorAccess)`

**Parameters:**
- `authorAccess` ([`ro.sync.ecss.extensions.api.AuthorAccess`](../api/AuthorAccess.md)): The author access of the XML document.

**Returns:** `java.net.URL`

