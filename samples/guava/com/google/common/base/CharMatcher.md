...
Determines a true or false value for any Java `char` value, just as `Predicate` does
 ...
 <p><b>Warning:</b> This class deals only with `char` values, that is, <a href="http://www.unicode.org/glossary/#BMP_character">BMP characters</a>. It does not understand
 ...
 <p>See the Guava User Guide article on <a href="https://github.com/google/guava/wiki/StringsExplained#charmatcher">`CharMatcher`
 </a>.
...
Returns a `char` matcher that matches any character except the BMP character specified.
 ...
 <p>To negate another `CharMatcher`, use `negate()`.
...
Returns a `char` matcher functionally equivalent to this one, but which may be faster to
 ...
 <p>This method has no effect (returns `this`) when called in GWT: it's unclear whether a
...
Returns `true` if a character sequence contains at least one matching BMP character.
 ...
 <p>The default implementation iterates over the sequence, invoking `matches` for each
...
Returns the index of the first matching BMP character in a character sequence, or `-1` if
 ...
 <p>The default implementation iterates over the sequence in forward order calling `matches` for each character.
...
Returns the index of the first matching BMP character in a character sequence, starting from a
 ...
 <p>The default implementation iterates over the sequence in forward order, beginning at `start`, calling `matches` for each character.
...
Returns the index of the last matching BMP character in a character sequence, or `-1` if
 ...
 <p>The default implementation iterates over the sequence in reverse order calling `matches` for each character.
...
Returns the number of matching `char`s found in a character sequence.
 ...
 <p>Counts 2 per supplementary character, such as for `whitespace`().`negate`().
...
Returns a string copy of the input character sequence, with each matching BMP character
 ...
 <p>The default implementation uses `indexIn(CharSequence)` to find the first matching
