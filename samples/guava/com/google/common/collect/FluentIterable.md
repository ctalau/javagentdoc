...
A discouraged (but not deprecated) precursor to Java's superior {@link Stream} library.
 ...
 <p>Several lesser-used features are currently available only as static methods on the {@link Iterables} class.
...
Returns a fluent iterable that wraps {@code iterable}, or {@code iterable} itself if it is
 ...
 <p><b>{@code Stream} equivalent:</b> {@link Collection#stream} if {@code iterable} is a {@link Collection}; {@link Streams#stream(Iterable)} otherwise.

### `from([`E[]`](E.md) elements)`
...
Returns a fluent iterable containing {@code elements} in the specified order.
 ...
 <p><b>{@code Stream} equivalent:</b> {@link java.util.stream.Stream#of(Object[]) Stream.of(T...)}.
...
### `of([`E`](E.md) element, [`E[]`](E.md) elements)`
...
Returns an {@link Optional} containing the first element in this fluent iterable. If the
 ...
 <p><b>{@code Stream} equivalent:</b> if the goal is to obtain any element, {@link Stream#findAny}; if it must specifically be the <i>first</i> element, {@code Stream#findFirst}.
...
@throws NullPointerException if the last element is null; if this is a possibility, use {@link Iterables#getLast} instead.
...
### `toSortedList([`java.util.Comparator<? super E>`](../../../../java/util/Comparator.md) comparator)`
...
### `toSortedSet([`java.util.Comparator<? super E>`](../../../../java/util/Comparator.md) comparator)`
...
Returns a map with the contents of this {@code FluentIterable} as its {@code values}, indexed
 ...
 <p>If your index may associate multiple values with each key, use {@link #index(Function) index}.
...
**Returns:** [`java.util.stream.Stream<E>`](../../../../java/util/stream/Stream.md)

Returns a stream of this fluent iterable's contents (similar to calling {@link Collection#stream} on a collection).
