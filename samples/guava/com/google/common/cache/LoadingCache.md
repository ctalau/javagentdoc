...
A semi-persistent mapping from keys to values. Values are automatically loaded by the cache, and
 ...
 <p>When evaluated as a {@link Function}, a cache yields the same result as invoking {@link #getUnchecked}.
...
@throws ExecutionException if a checked exception was thrown while loading the value. ({@code
     ExecutionException} is thrown <a href="https://github.com/google/guava/wiki/CachesExplained#interruption">even if
...
Returns a map of the values associated with {@code keys}, creating or retrieving those values
 ...
 <p>Caches loaded by a {@link CacheLoader} will issue a single request to {@link CacheLoader#loadAll} for all keys which are not already present in the cache. All entries
...
@deprecated Provided to satisfy the {@code Function} interface; use {@link #get} or {@link #getUnchecked} instead.
@throws UncheckedExecutionException if an exception was thrown while loading the value. (As
